##### 勒贝格积分
- 勒贝格积分
	- **勒贝格积分**扩展了传统的[[积分|黎曼积分]], 使其能够处理更广泛的函数和集合. 勒贝格积分的基本思想是通过对函数值进行加权, 使用测度来替代区间长度, 按照函数的值域划分, 而非定义域划分, 因此比黎曼积分更具灵活性和强大能力, 特别是在处理不连续函数和具有不规则行为的函数时. 勒贝格积分仅对[[测度空间]]上的[[可测函数]]定义, 最简单的就是[[简单函数]]
