##### 线性算子
- 线性算子 [[线性算子的矩阵|矩阵]]
	- **线性算子** $T$ 是向量空间 $V$ 映射到其自身的[[线性变换]] $T: V \to V$ , 其矩阵是方阵 $A$, 同一算子关于不同基的矩阵表示之间是[[矩阵相似]]的. [[算子的幂]]即方阵的幂. 一般的算子类型有[[恒等算子]], [[可逆算子]], [[幂零算子]], [[可交换算子]], [[可上三角化算子]], [[可对角化算子]], [[可若尔当化算子]]. 还有内积空间上的[[可正交对角化算子]]等
