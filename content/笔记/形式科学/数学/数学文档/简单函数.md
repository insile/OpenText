##### 简单函数
- 简单函数
	- **简单函数**是由可测集确定的[[阶跃函数]], 可以认为是一个分段常值函数. 设[[可测空间]] $(X,\mathcal{A})$ , [[可测集|不相交可测集]][[序列]] $A_1,\dots,A_n\in \mathcal{A}$ , 实数或复数序列 $a_1,\dots,a_n\in\mathbb{F}$, 则一个简单函数就是一个映射 $f:X\to\mathbb{F}$ , 是可测集[[指示函数]] ${\mathbf {1} }_{A_{i}}(x)$ 的有限[[线性组合]]
