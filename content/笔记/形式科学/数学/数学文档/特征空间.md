##### 特征空间
- 特征空间
	- **特征空间** $E(\lambda, T)$ 是[[线性算子]] $T$ 在[[特征值和特征向量|特征值]] $\lambda$ 下的[[不变子空间]], 由零向量 $\mathbf{0}$ 和相应的特征向量 $\mathbf{v}$ 构成, 是特征向量的[[张成空间]], 特征空间的维数称为特征值的[[特征值的重数|几何重数]], 互异特征值对应的特征空间之和是[[子空间的直和]]
