##### 特征值分解
- 特征值分解 (对角化)
	- $A=PDP^{-1}$, 特征值分解是将矩阵分解为由其特征值构成的对角矩阵 $D$ 和特征向量构成的可逆矩阵 $P$. 只有可[[可对角化矩阵]]的矩阵才可特征值分解
...
- 对称矩阵特征值分解 (谱分解)
	- $A=PDP^{-1}=PDP^T$ , 对称矩阵特征值分解是将对称矩阵分解为由其特征值构成的对角矩阵 $D$ 和标准正交特征向量构成的正交矩阵 $P$, 即[[可对角化矩阵|正交对角化]]. 可谱分解为一组特征值与特征向量的乘积, 
