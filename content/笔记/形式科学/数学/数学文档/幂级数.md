##### 幂级数
- 幂级数
	- **幂级数**是指每一项都是[[幂函数]]的[[函数项级数]], 一个关于变量 $x$ 的幂级数一般定义为 $\sum^{\infty}_{n=0}a_n(x-x_0)^n$, 其中 $a_n$ 为系数, $x_0$ 为中心点, 幂级数在中心点必定收敛, 其收敛范围由[[柯西-阿达玛定理|收敛半径]] $R$ 决定. 幂级数在收敛区间内[[绝对收敛]], 并在任意紧子区间内[[一致收敛]], 在边界点满足[[阿贝尔极限定理]]. 光滑函数可[[幂级数展开|展开]]为[[泰勒级数]], 从而定义[[解析函数]]. 博雷尔定理表明每个幂级数都是某个光滑函数的泰勒级数
