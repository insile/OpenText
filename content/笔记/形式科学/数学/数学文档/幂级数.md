##### 幂级数
- 幂级数
	- **幂级数**是指每一项都是[[幂函数]]的[[函数项级数]], 一个关于变量 $x$ 的幂级数一般定义为 $\sum^{\infty}_{n=0}a_n(x-x_0)^n$, 其中 $a_n$ 为系数, $x=x_0$ 为中心点, 幂级数在中心点必定收敛, 中心点周围收敛的范围称为[[柯西-阿达玛定理|收敛半径]] $R$. 幂级数在收敛区间内[[绝对收敛]], 并在任意紧子区间内[[一致收敛]], 具有一些[[幂级数运算]]. 可以将函数[[幂级数展开|展开]]成幂级数, 即[[泰勒级数]], 并且定义[[解析函数]]
