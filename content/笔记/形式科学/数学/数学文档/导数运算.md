##### 导数运算
- 导数运算
	- **导数运算**是对[[导数]]或[[微分]]进行的[[运算]]和[[运算律]], 主要是与代数运算的可交换性和一些计算定理, 有[[高阶导数]], [[初等函数微积分|初等函数导数表]], [[微积分基本定理]]等
		- 和差
		...
		- 隐函数求导
			- $\displaystyle \frac{dy}{dx} = -\frac{F_x}{F_y}$
				...
				- $\displaystyle \frac{dy}{dx} = -\frac{F_x}{F_y}$, $\displaystyle F_x = \frac{\partial F}{\partial x}$, $\displaystyle F_y = \frac{\partial F}{\partial y}$
		- 反函数求导
			- $\displaystyle \frac{dx}{dy} = \frac{1}{f'(x)}$
				- 求导 $y = f(x)$
