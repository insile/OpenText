##### 内积
- 内积
	- **内积**是定义在[[向量空间]] $V$ 上的[[运算|二元运算]] $\langle \cdot ,\cdot \rangle:V\times V\rightarrow\mathbb{F}$, 对任意 $\mathbf{u}, \mathbf{v},\mathbf{w}\in V$, $\alpha,\beta\in\mathbb{F}$ 满足以下公理, 使空间成为[[内积空间]]. 具体来说[[实数内积]]是双线性型, [[复数内积]]是埃尔米特型. 内积在任意[[向量空间的基|基]]下坐标表示为[[格拉姆矩阵]], 默认使用[[正交基|标准正交基]]简化计算. 内积测量两个向量的方向相似度, 表示一个向量在另一个向量上的[[正交投影|正交投影长度]], 通过固定第二个向量可[[里斯表示定理|表示]]线性泛函. 内积可诱导[[范数]]和[[度量]], 赋予空间几何结构. 内积可推广为流形上的[[度量张量]]
		- 线性, 第一变量
