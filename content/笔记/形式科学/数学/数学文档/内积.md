##### 内积
- 内积
	- **内积**是定义在[[向量空间]] $V$ 上的[[运算|二元运算]] $\langle \cdot ,\cdot \rangle:V\times V\rightarrow\mathbb{F}$, 使之成为[[内积空间]]. 具体来说[[实数内积]]是双线性型, [[复数内积]]是埃尔米特型. 一般通过矩阵计算内积是在[[正交基|标准正交基]]上进行. 对任意 $\mathbf{u}, \mathbf{v},\mathbf{w}\in V$, $\alpha,\beta\in\mathbb{F}$, 存在一个数 $\langle\mathbf{u},\mathbf{v}\rangle$ 满足下面公理. 内积可导出[[范数]]和[[度量]]
