##### 埃尔米特型的矩阵
- 埃尔米特型的矩阵
	- [[埃尔米特型]] $H$ 在 $V$ 中基 $\{e_1, e_2, \dots, e_n\}$ 下的矩阵 $A$ 的元素 $A_{ij}$​ 可以通过以下方式定义 $a_{ij} = H(e_i, e_j)$ , 类似于[[格拉姆矩阵]]也是[[埃尔米特矩阵]], 并且同一埃尔米特型关于不同基的矩阵表示之间是[[矩阵合同]]的
