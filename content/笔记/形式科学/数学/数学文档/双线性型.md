##### 双线性型
- 双线性型 [[双线性型的矩阵|矩阵]]
	- **双线性型** $B$ 是定义在一个[[向量空间]]上的[[双线性泛函]] $B:V\times V\rightarrow\mathbb{F}$ , 满足双线性性, 单独对两个变量都是线性的, $V$ 上双线性型构成的集合记为 $V^{(2)}$. [[二次型|实二次型]]是一个向量上的对称双线性型, [[实数内积]]是正定对称双线性型, 双线性型可推广为[[多重线性型]]
