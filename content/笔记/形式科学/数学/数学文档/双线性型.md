##### 双线性型
- 双线性型
	- **双线性型**是定义在两个[[向量空间]]上的[[多重线性型]] $B:V\times V\rightarrow\mathbb{F}$, 对所有 $\mathbf{u}, \mathbf{v}, \mathbf{w} \in V$, $\alpha, \beta \in \mathbb{F}$ 满足双线性性等条件, 在任意基下表示为[[方阵]], 对称双线性型是[[对称矩阵]], 交错双线性型是[[反对称矩阵]], 同一双线性型关于不同基的矩阵表示之间是[[矩阵合同]]的. 特别的, [[二次型|实二次型]]是一个向量上的对称双线性型, [[实数内积]]是正定对称双线性型
