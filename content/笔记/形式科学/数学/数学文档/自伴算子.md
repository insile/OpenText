##### 自伴算子
- 自伴算子 [[自伴算子的矩阵|矩阵]]
	- **自伴算子** $T$ 是定义在[[内积空间]] $V$ 上的[[线性算子]], 满足与自身[[伴随映射|伴随]]相等 $T=T^∗$ , 这意味着对于所有 $\mathbf{u},\mathbf{v}\in V$ 都有 $\langle T(\mathbf{u}),\mathbf{v}\rangle=\langle\mathbf{u},T(\mathbf{v})\rangle$
		- $T=T^∗$, $A=A^*$
- 自伴算子性质与条件
