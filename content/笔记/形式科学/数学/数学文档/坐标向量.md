##### 坐标向量
- 坐标向量
	- **坐标向量** $[\mathbf{a}]_\mathcal{B}\in\mathbb{F}^n$ 是[[向量]]在某个特定[[向量空间的基|基]]或者说[[向量空间的坐标系|坐标系]]下[[线性组合]]的数值表达, 通常用[[向量矩阵]]表示, 向量 $\mathbf{a}$ 是一个独立于坐标系的[[向量空间]]的元素, 向量本身没有固定的坐标表示, 只有当我们选择了基或者说坐标系 $\mathcal{B}$ 后, 才能为它指定一个具体的坐标向量, 坐标向量依赖于选择的基底和向量空间的域, 不同的基底和域会给同一个向量不同的坐标表示. **很多时候向量指的是标准基上的坐标向量**
