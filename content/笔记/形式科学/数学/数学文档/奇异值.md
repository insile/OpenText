##### 奇异值
- 奇异值
	- **奇异值**是将一般的[[线性变换]]化为[[正算子]]后的[[特征值和特征向量|特征值]]的平方根. 设线性变换 $T\in L(V,W)$, 则有特征值非负的正算子 $T^*T$, 所以 $T$ 的奇异值 $\sigma$ 是 $T^*T$ 的特征值的非负平方根  $\sigma=\sqrt{\lambda}$, 按降序排列, 而且每个奇异值的出现次数, 等于 $T^*T$ 对应[[特征空间]]的维数, 有[[奇异值分解]]
