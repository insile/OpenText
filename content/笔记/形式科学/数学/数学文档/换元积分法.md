##### 积分换元法
- 积分换元法
	- **积分换元法**是一种[[积分]]运算方法. 通过引入新的变量将复杂的积分转化为较简单的形式, 从而便于求解. 第一类换元法也称为凑微分法, 其核心思想是通过将[[反导数|不定积分]]被积函数的一部分凑成某个函数的[[微分]], 从而简化积分. 第二类换元法也称为变量代换法, 与第一类换元法思想相反, 其核心思想是通过引入新的变量替换不定积分被积函数中的复杂部分, 从而简化积分. 
		- 凑微分法, 设 $u=g(x)$, 则 $\text{d}u=g'(x)\text{d}x$, 如果被积函数可以表示为 $f(g(x))g'(x)$, 则 $\int f(g(x))g'(x){\rm d}x=\int f(u)\text{d}u$. 定积分要处理上下限. 令 $u=g(x)$ , 且 $g'$ 在 $[a,b]$ 内连续, $f$ 在 $u=g(x)$ 值域内连续, 则可换元
		...
		- 变量代换法, 设 $x=g(u)$, 则 $\text{d}x=g'(u)\text{d}u$, 如果被积函数可以表示为 $f(x)$, 则 $\int f(x){\rm d}x=\int f(g(u))g'(u){\rm d}u$. 定积分要处理上下限. 令 $x=g(u)$ , 且 $g$ 在 $[a,b]$ 内可导且单调, 反函数 $g^{-1}(x)$ 存在, 则可换元
