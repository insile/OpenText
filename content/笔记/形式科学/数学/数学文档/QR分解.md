##### QR分解
- QR分解
	- **QR分解**指[[方阵]] $A$ 分解为[[正交矩阵]]或者[[酉矩阵]] $Q$ 和[[三角矩阵|上三角矩阵]] $R$, 即 $A = QR$. 若 $A$ [[矩阵的秩|列满秩]], 则可QR分解, 首先通过[[格拉姆-施密特方法]]求 $A$ [[列空间]]的[[正交基|标准正交基]]组成 $Q$ , 然后通过 $A = QR\rightarrow Q^*A=Q^*QR=IR=R$ 求出$R$
		- $A_{n\times n}$ 方阵
