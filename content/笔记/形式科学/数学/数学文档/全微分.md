##### 全微分
- 全微分
	- **全微分**是对[[实函数|多元函数]]值在任意方向变化量的线性近似, 是一元函数[[微分]]的推广, 如果与自变量变化量比值存在, 则等于[[方向导数]]. 全微分是可微函数所有自变量的变化引起的函数值的总变化, 如果只有一个变化就是[[偏微分]], 全微分通过所有偏导数加权相应变量的变化量来计算, 即[[梯度]]对自变量微分的[[线性变换]]
