##### 矩阵合同
- 矩阵合同
	- **矩阵合同** $A\simeq B$ 指对于[[方阵]] $A$ 与 $B$ 存在[[可逆矩阵|可逆矩阵]] $P$ 使得 $P^{T}AP=B$ . 矩阵合同的本质在于它们代表的是同一个[[双线性型]]关于不同基底的矩阵, 等式 $P^{T}AP=B$ 也称为双线性型的换基公式, $P$ 为[[基的变换|过渡矩阵]]. 所以合同矩阵共有同一双线性型的性质. 常用矩阵合同有[[二次型的标准形]]
