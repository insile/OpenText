##### LU分解
- LU分解
	- LU分解指[[方阵]] $A_{n\times n}$ 可以分解为对角线上的元素为 $1$ 的[[三角矩阵|下三角矩阵]] $L_{n\times n}$ 和[[三角矩阵|上三角矩阵]] $U_{n\times n}$ , 即 $A = LU$ . 本质是没有对换变换的[[初等变换]]消元法, 充要条件是消去第 $i$ 列时的 $a_{ii}$ 不为零, $A$ 顺序主子式全不为 $0$
- $A = LU$
	- $A_{n\times n}$ [[方阵]]
