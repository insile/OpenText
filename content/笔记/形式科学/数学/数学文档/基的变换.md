##### 基的变换
- 基的变换
	- 设 $V$ 是一个[[向量空间]], $B = \{\mathbf{b}_1, \mathbf{b}_2, \cdots, \mathbf{b}_n\}$ 和 $C = \{\mathbf{c}_1, \mathbf{c}_2, \cdots, \mathbf{c}_n\}$ 是 $V$ 中的两个[[向量空间的基|基]]或者说[[向量空间的坐标系|坐标系]], **基的变换**指更改基使得向量 $\pmb{x}$ 从一个[[坐标向量]] $[\pmb{x}]_B$ 映射到另一个 $[\pmb{x}]_C$ 的[[坐标变换|线性坐标变换]], 也是一个[[线性变换|可逆线性变换]], 过渡矩阵 $P_{B\to C}$ 是基的变换的[[矩阵变换]]表示, 即 $[\pmb{x}]_C=P_{B\to C}[\pmb{x}]_B$ , 并且  $P_{B\to C}$ 的列是基 $B$ 在基 $C$ 下的坐标向量, 即 $P_{B\to C}=\begin{bmatrix} [\pmb{b}_1]_C & [\pmb{b}_2]_C & \cdots & [\pmb{b}_n]_C \end{bmatrix}$ , 而逆变换即逆矩阵
