##### 伴随映射
- 伴随映射 [[伴随映射的矩阵|矩阵]]
	- **伴随映射** $T^*:W\rightarrow V$ 是一个定义在[[内积空间]]上的[[线性变换]], 是通过保持[[内积]]不变来定义, 即使得对任意 $\mathbf{v}\in V$, $\mathbf{w}\in W$ 有 $\langle T(\mathbf{v}),\mathbf{w}\rangle=\langle\mathbf{v},T^*(\mathbf{w})\rangle$
