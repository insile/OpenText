##### 雅可比矩阵
- 雅可比矩阵
	- **雅可比矩阵**是[[实函数|多元向量函数]]的一阶[[偏导数]]排列成的[[矩阵]]. 矩阵本身可以表示[[矩阵变换]], 而雅可比矩阵是对多元向量函数的局部线性近似, 是一元函数切线斜率[[导数|导数]]的推广. 可以说导数被重新解释为[[线性变换]]. 可用于[[隐函数定理]], [[反函数定理]]. 对于方阵雅可比矩阵, 可以计算[[行列式]], 称为雅可比行列式, 雅可比行列式描述了空间变换的变化程度, 可用于[[重积分变量替换]]
