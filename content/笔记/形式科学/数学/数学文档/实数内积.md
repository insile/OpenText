##### 实数内积
- 实数内积
	- **实数内积**是有限维[[实向量空间]]的[[内积]] $\langle \cdot ,\cdot \rangle:V\times V\rightarrow\mathbb{R}$, 使得实向量空间成为一个[[欧氏空间]], 对所有 $\mathbf{u}, \mathbf{v}, \mathbf{w} \in V$, $\alpha, \beta \in \mathbb{R}$ 满足[[双线性型|正定对称双线性型]], 并且由双线性型的[[双线性型的矩阵|矩阵]]可得实数内积在任意基下表示为 $\langle \mathbf{x}, \mathbf{y} \rangle = \mathbf{x}^T A \mathbf{y}$, $A$ 是[[定性矩阵|正定对称矩阵]], 在标准正交基表示为 $\langle \mathbf{x}, \mathbf{y} \rangle = \mathbf{x}^T \mathbf{y}$
		- 双线性性
