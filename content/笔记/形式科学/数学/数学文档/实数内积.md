##### 实数内积
- 实数内积
	- **实数内积**是定义在[[实向量空间]]上的[[内积]] $\langle \cdot ,\cdot \rangle:V\times V\rightarrow\mathbb{R}$, 对任意 $\mathbf{u}, \mathbf{v}, \mathbf{w} \in V$, $\alpha, \beta \in \mathbb{R}$ 满足[[双线性型|正定对称双线性型]], 使空间成为[[欧氏空间]]. 实数内积在任意基下表示为 $\langle \mathbf{x}, \mathbf{y} \rangle = \mathbf{y}^T A \mathbf{x}$, 其中 $A$ 是[[定性矩阵|正定]][[对称矩阵]], 而在标准正交基下简化为 $\langle \mathbf{x}, \mathbf{y} \rangle = \mathbf{y}^T \mathbf{x}$
		- 双线性性
...
>	- $x^T A x > 0$
>- 所以对于一般基
>	- $\langle \mathbf{x}, \mathbf{y} \rangle=B(\mathbf{x}, \mathbf{y}) = \mathbf{y}^T A \mathbf{x}$ , $A=A^T$
>- 常用对于标准正交基 $A=I$
>	- $\langle \mathbf{x}, \mathbf{y} \rangle=B(\mathbf{x}, \mathbf{y}) = \mathbf{y}^T  \mathbf{x}$
