##### 对偶映射的矩阵
- 对偶映射的矩阵
	- **对偶映射的矩阵**是原[[线性变换]]矩阵的[[矩阵转置|转置]]. 设[[线性变换]] $T: V \to W$, 矩阵表示为 $A$, 设[[对偶映射]] $T': W' \to V'$, 矩阵表示为 $C$, 则 $C=A^T$
		- $T: V \to W$, $A$
		- $T': W' \to V'$, $C=A^T$
	- 设 $V$ 和 $W$ 的[[向量空间的基|基]]为 $\{\mathbf{v}_1, \mathbf{v}_2, \dots, \mathbf{v}_n\}$, $\{\mathbf{w}_1, \mathbf{w}_2, \dots, \mathbf{w}_m\}$, 设 $V'$ 和 $W'$ 的[[对偶基]]为 $\{\phi_1, \phi_2, \dots, \phi_n\}$, $\{\psi_1, \psi_2, \dots, \psi_m\}$. 对偶映射可表示为 $T' (\psi) =\psi\circ T$, 并且每个对偶基向量满足克罗内克函数
