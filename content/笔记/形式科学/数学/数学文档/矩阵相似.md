##### 矩阵相似
- 矩阵相似
	- **矩阵相似**是一种[[矩阵关系]], 记作 $A\sim B$, 指对于[[方阵]] $A$ 与 $B$ 存在[[可逆矩阵]] $P$ 使得 $P^{-1}AP=B$. 矩阵相似的本质在于它们代表同一个[[线性算子]]关于不同基底的矩阵, 等式 $P^{-1}AP=B$ 也称为线性算子的换基公式, $P$ 为[[基的变换|过渡矩阵]]. 所以相似矩阵共有同一线性算子的性质, 例如[[矩阵的秩|秩]], [[特征值和特征向量|特征值]], [[矩阵的迹|迹]], [[行列式]], [[特征多项式]]都相同. 常用矩阵相似有[[可上三角化矩阵]], [[可对角化矩阵]], [[可若尔当化矩阵]], [[可正交对角化矩阵]]
