##### 矩阵相似
- 矩阵相似
	- 矩阵相似 $A\sim B$ 指对于[[方阵]] $A$ 与 $B$ 存在[[可逆矩阵|可逆矩阵]] $P$ 使得 $P^{-1}AP=B$ . 矩阵相似的本质在于它们代表的是同一个[[线性算子]]关于不同基底的矩阵, 等式 $P^{-1}AP=B$ 也称为线性算子的换基公式, $P$ 为[[基的变换|过渡矩阵]]. 所以相似矩阵共有同一算子的性质, 例如[[矩阵的秩]]相等, [[特征空间|矩阵的特征值]]相同. 如果能与对角矩阵相似, 则称[[可对角化矩阵]], 即[[可对角化算子]]
