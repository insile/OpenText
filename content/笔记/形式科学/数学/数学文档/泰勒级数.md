##### 泰勒级数
- 泰勒级数
	- **泰勒级数**是特殊的[[幂级数]], 其系数从[[导数]]构造, 即[[幂级数展开]]. 设函数 $f(x)$ 在点 $x_0$​ 处有任意阶导数, 则泰勒级数定义为 $f(x) = \sum_{n=0}^{\infty} \frac{f^{(n)}(x_0)}{n!} (x-x_0)^n$, 其中 $f^{(n)}(x_0)$ 表示 $f(x)$ 在 $x_0$​ 处的 $n$ 阶导数, $n!$ 为阶乘. 当 $x_0=0$ 时称为麦克劳林级数. [[泰勒展开式]]是将函数写成泰勒级数的形式. [[泰勒公式]]是将函数写成泰勒多项式与余项的和以便进行误差控制
