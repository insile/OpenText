##### 曲面面积
- 曲面面积
	- **曲面面积**是指[[参数化曲面|曲面]]在某个区域中的面积, 通过对曲面进行无限分割, 将每个小部分的面积近似为平面区域的面积, 然后通过积分求和得到整个曲面的面积. 设光滑曲面参数方程 $\mathbf{r}(u,v)$, 曲面面积面积可以用[[二重积分]]表示, 其中[[叉积]]范数等于以两个[[切向量]]为边的平行四边形的面积, 用作近似
		- $\displaystyle S = \iint_D ||\mathbf{r}_u \times \mathbf{r}_v || {\rm d}u{\rm d}v$
		- $\displaystyle {\rm d}S= ||\mathbf{r}_u \times \mathbf{r}_v || {\rm d}u{\rm d}v$
