---
tags:
  - 首字母/T
  - 级别/托福
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ˈtæli/； 美：/ˈtæli/
- #词性/v  (说法、数字等)与…符合(或一致)；吻合；计算(总的数目、成本等)；计数；合计
- #词性/n  记录；账；积分表
# 例句
- The card counter to keep a tally ?
	- 算牌的人就可以计数了吗？
- Therefore , the volume down to the topmost perforation must be accurately known ( from the tally and surface volume of pipework ) .
	- 因此，（根据管道系统的计数和表面积）准确了解下降到最顶层穿孔的体积。
- Keep a tally of how much you spend while you 're away .
	- 在外出期间，把你的花费都记录下来。
# 形态
- #形态/word_third tallies
- #形态/word_ing tallying
- #形态/word_done tallied
- #形态/word_pl tallies
- #形态/word_past tallied
