---
tags:
  - 首字母/T
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/tɑːn/； 美：/tɑːrn/
- #词性/n  山中小湖；冰川湖
# 例句
- The deep and dark Tarn at my feet closed over the fragments of the house of usher .
	- 我脚下深邃阴沉的小湖将厄谢尔古屋的断垣残墙吞没了。
- This pool or tarn was encircled by tree !
	- 这个池塘，或是说山潭吧，四周全被树木围了起来。
- The first one extends the method of feedback linearization proposed by T. J. Tarn , et al for robot to uncertain case .
	- 第一种算法将T.J.Tarn（谈自忠）等提出的机器人反馈线性化控制推广到存在不确定性的情形。
# 形态
- #形态/word_pl tarns
