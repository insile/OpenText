---
tags:
  - 首字母/P
  - 级别/中考
掌握: false
模糊: false
---
# 词义
- 英：/ˈprəʊɡræm/； 美：/ˈproʊɡræm/
- #词性/n  方案；计划；（演出或活动的）程序；程序；（机器工作的）程序；课程；节目；节目单；活动安排；演出说明书
- #词性/vt  计划；规划；安排；训练；培养；预设；预调
# 例句
- Select a cool programme for woollen clothes .
	- 洗毛衣要选择凉水程序。
- The new system may be more economic but will lead to a decline in programme quality .
	- 新系统可能更合算，但是会降低程序质量。
- The survey formed part of a larger programme of research .
	- 这个调查是研究计划的一部分。
# 形态
- #形态/word_third programmes
- #形态/word_ing programming
- #形态/word_done programmed
- #形态/word_pl programmes
- #形态/word_past programmed
