---
tags:
  - 首字母/C
  - 级别/高考四级
掌握: false
模糊: false
---
# 词义
- 英：/ˈkætəlɒɡ/； 美：/ˈkætəlɔːɡ/
- #词性/n  目录；目录簿；一连串(糟糕)事
- #词性/vt  记载，登记(某人、某事等的详情)；编入目录；列入目录
# 例句
- We will be forwarding you our new catalogue next week .
	- 我们将于下星期给你寄上新的商品目录。
- The catalogue gives a full description of each product .
	- 该目录对每一种产品都作了详尽的说明。
- We will be forwarding our new catalogue to you next week .
	- 我们将于下星期给你寄上新的商品目录。
# 形态
- #形态/word_third catalogues
- #形态/word_ing cataloguing
- #形态/word_done catalogued
- #形态/word_pl catalogues
- #形态/word_past catalogued
