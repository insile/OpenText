---
tags:
  - 首字母/H
  - 级别/高考四级
掌握: false
模糊: false
---
# 词义
- 英：/ˌhɒrɪˈzɒntl/； 美：/ˌhɔːrɪˈzɑːntl/
- #词性/adj  水平的；横的；与地面平行的
- #词性/n  水平面；水平线；水平位置；横线；横切面
# 例句
- Wide horizontal tabbed folders work better than narrow vertical ones because they allow for greater expansion of tab names .
	- 选项卡为水平的文件夹比选项卡为垂直的文件夹效果更好，因为它们允许扩展更长的选项卡名。
- The taxi head rest advertisement frame comprises a horizontal mounting panel and a vertical board which is vertical to the mounting panel .
	- 该出租车座椅头枕广告架包括有一个水平的安装板和与安装板垂直的竖板；
- The board consists of vertical and horizontal lines
	- 板上画着横纵的线条。
# 形态
- #形态/word_pl horizontals
