---
tags:
  - 首字母/B
  - 级别/高考四级
掌握: false
模糊: false
---
# 词义
- 英：/ˈbrɪsl/； 美：/ˈbrɪsl/
- #词性/n  刚毛；刷子毛；短而硬的毛发
- #词性/vi  (背部或颈部的毛因惊吓或发怒)竖起，耸起；被激怒；(对某人的言行)大为恼怒
# 例句
- The results showed that stoma and bristle number on upper leaves surface is not related with resistance .
	- 叶片正面气孔数目、刚毛数与品种抗病性不相关。
- Result : The texture of integumentary cells , the shape and size of hair socket and bristle were important characters for species identification .
	- 结果：可根据体壁细胞的纹理、毛窝的形态、大小及刚毛的特征进行区别。
- The whole subject bristles with problems .
	- 整个事情问题成堆。
# 形态
- #形态/word_third bristles
- #形态/word_ing bristling
- #形态/word_done bristled
- #形态/word_pl bristles
- #形态/word_past bristled
