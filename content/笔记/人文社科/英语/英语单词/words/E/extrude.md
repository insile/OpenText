---
tags:
  - 首字母/E
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ɪkˈstruːd/； 美：/ɪkˈstruːd/
- #词性/v  (把金属或塑料)挤压成，压制；(被)挤压出，排出，喷出
# 例句
- Finally , after trial , the die can extrude qualified part .
	- 最后通过试模后，该模具能够挤压出完全合格的铝型材。
- The oocytes undergoing meiotic division sequentially extrude the first and the second small polar bodies from the large oocytes .
	- 卵母细胞在减数分裂过程中相继排出第一和第二极体。
- Lava is extruded from the volcano .
	- 熔岩从火山中喷出。
# 形态
- #形态/word_third extrudes
- #形态/word_ing extruding
- #形态/word_done extruded
- #形态/word_past extruded
