---
tags:
  - 首字母/E
  - 级别/托福
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ɪkˈstræpəleɪt/； 美：/ɪkˈstræpəleɪt/
- #词性/v  外推；推断；推知
# 例句
- It is unhelpful to extrapolate general trends from one case .
	- 根据一个案例来推断总的趋势是没有用的。
- What kind of information can you extrapolate from team velocity ?
	- 可以从团队进度中推断出哪些信息。
- We have extrapolated these results from research done in other countries .
	- 我们从其他国家所作的研究中推断出这些结果。
# 形态
- #形态/word_third extrapolates
- #形态/word_ing extrapolating
- #形态/word_done extrapolated
- #形态/word_past extrapolated
