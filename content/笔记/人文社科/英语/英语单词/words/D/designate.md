---
tags:
  - 首字母/D
  - 级别/考研
  - 级别/六级
  - 级别/雅思
  - 级别/托福
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ˈdezɪɡneɪt/； 美：/ˈdezɪɡneɪt/
- #词性/vt  指定；命名；选定，指派，委任(某人任某职)；指明；标示；标明
- #词性/adj  (已受委派)尚未上任的；(已当选)尚未就职的
# 例句
- Apraxia of speech is a term frequently used by speech pathologists to designate an impairment in the voluntary production of articulation and prosody ( the rhythm and timing ) of speech .
	- 言语失用常被言语治疗师用来命名随意言语的清晰度和韵律（节奏和时间）障碍。
- You can designate a default database reference for your database project .
	- 可以指定数据库项目的默认数据库引用。
- This area has been designated a National Park .
	- 本区已定为国家公园。
# 形态
- #形态/word_third designates
- #形态/word_ing designating
- #形态/word_done designated
- #形态/word_past designated
