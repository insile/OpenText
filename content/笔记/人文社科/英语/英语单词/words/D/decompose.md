---
tags:
  - 首字母/D
  - 级别/六级
  - 级别/雅思
  - 级别/托福
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ˌdiːkəmˈpəʊz/； 美：/ˌdiːkəmˈpoʊz/
- #词性/v  (使)分解；腐烂
# 例句
- The fertiliser releases nutrients gradually as bacteria decompose it .
	- 随着细菌的分解，肥料逐渐释放出各种营养成分。
- All substances , except those which decompose when heated , may be changed from one state into another .
	- 所有物质，除了一加热就分解的以外，都可以以一种状态变为另一种状态。
- As the waste materials decompose , they produce methane gas .
	- 废物腐烂时会产生沼气。
# 形态
- #形态/word_third decomposes
- #形态/word_ing decomposing
- #形态/word_done decomposed
- #形态/word_past decomposed
