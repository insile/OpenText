---
tags:
  - 首字母/D
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ˈdæli/； 美：/ˈdæli/
- #词性/v  磨蹭，拖拉；调情；嬉戏，调戏；延误；轻率地对待；游荡，闲混；（途中）逗留；蹉跎(时光)
# 例句
- Through input-output principle , dally fee is added up to maintenance cost model , which makes pavement decision-making assessment system more rational .
	- 并应用输入～输出法原理，在养护费用模型中提出增加延误费，使路面评价决策系统更加合理。
- The bureaucrats dallied too long
	- 这些官僚拖拉了太久。
- He did not dally over the choice of a partner .
	- 他没有在选择合伙人的问题上浪费时间。
# 形态
- #形态/word_third dallies
- #形态/word_ing dallying
- #形态/word_done dallied
- #形态/word_past dallied
