---
tags:
  - 首字母/D
  - 级别/托福
掌握: false
模糊: false
---
# 词义
- 英：/dɪˈfjuːzɪz/； 美：/dɪˈfjuːzɪz/
- #词性/v  (使气体或液体)扩散，弥漫，渗透；(使光)模糊，漫射，漫散；传播；使分散；散布；普及
# 例句
- While information diffuses in network , there is an acceptance threshold and interest threshold on one node for information .
	- 新模型从信息传播的本质出发，提出了结点对信息的兴趣度概念，信息在网络中传播时，结点对信息存在一个接受阈值和兴趣阈值。
- Indian Buddhism diffuses successfully in China by tasting to the psychological structure , way of life and value system rooted in Chinese shi class , which has a far-reaching influence on Chinese traditional culture .
	- 印度禅学通过对中国士大夫阶层心理结构、生活方式、价值取向等方面的策略性迎合，在这一群体中成功传播，进而影响中国文化千年之久。
- The problem is how to diffuse power without creating anarchy .
	- 问题在于如何将权力分散而不造成无政府状态。
# 形态
- #形态/word_proto diffuse
