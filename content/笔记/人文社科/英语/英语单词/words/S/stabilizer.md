---
tags:
  - 首字母/S
  - 级别/托福
掌握: false
模糊: false
---
# 词义
- 英：/ˈsteɪbəlaɪzə(r)/； 美：/ˈsteɪbəlaɪzər/
- #词性/n  稳定装置；(飞机的)安定面；(船舶的)减摇装置；(儿童自行车后轮两侧的)稳定轮；稳定剂
# 例句
- Principle and Dynamic Simulating Analysis of Power System Stabilizer with a Microcomputer
	- 微处理机电力系统受端稳定装置原理及动模试验分析
- The Development for Computer Simulation System of Crude Stabilizer
	- 原油稳定装置计算机仿真系统开发
- Abduction stabilizer bar between legs is never used as a handle to turn the child .
	- 在两腿之间有保持外展的支持棒，但千万不能用这条棒给小儿翻身。
# 形态
- #形态/word_pl stabilizers
