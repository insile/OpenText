---
tags:
  - 首字母/S
  - 级别/托福
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ˌsuːpərɪmˈpəʊz/； 美：/ˌsuːpərɪmˈpoʊz/
- #词性/vt  叠加；使重叠；使附加于；使(图像甲)叠映在(图像乙)上
# 例句
- Application of Superimpose Teaching Method in Teaching of Back Style High Jump
	- 叠加式教学法在背越式跳高教学中的运用
- MIMO channel estimation and performance analysis using superimpose training sequences
	- 基于叠加训练序列的MIMO信道估计及其性能分析
- She has tried to superimpose her own attitudes onto this ancient story .
	- 她想把自己的看法加入这个古老的故事里。
# 形态
- #形态/word_third superimposes
- #形态/word_ing superimposing
- #形态/word_done superimposed
- #形态/word_past superimposed
