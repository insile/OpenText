---
tags:
  - 首字母/U
  - 级别/GRE
掌握: false
模糊: false
---
# 词义
- 英：/ˈʌŋɡwənt/； 美：/ˈʌŋɡwənt/
- #词性/n  软膏；药膏
# 例句
- This unguent relieves rheumatism , chills and pains , and is fit for use By rheumatism sufferers .
	- 本药膏除湿散寒，祛风定痛，适合风湿病人使用。
- OBJECTIVE : To study the influence package on weight and appearance and active ingredient content of unguent .
	- 前言：目的：考察包装对医院外用软膏制剂重量、外观性状和有效成分含量的影响。
- The simultaneous determination of two components in benzoic acid co unguent using the rotating coordinate method
	- 变量轮换法同时测定复方苯甲酸软膏中二组分含量
