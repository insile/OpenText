...
##### 单射线性变换
- 单射线性变换
	- **单射线性变换**是指[[单射]]的[[线性变换]], 在任意基下表示为[[矩阵的秩|列满秩]][[矩阵]]. 设 $T: V \rightarrow W$ 是从向量空间 $V$ 到 $W$ 的线性变换, 若对于任意 $\mathbf{u}, \mathbf{v} \in V$, 当 $\mathbf{u} \neq \mathbf{v}$ 时必有 $T(\mathbf{u}) \neq T(\mathbf{v})$, 则称 $T$ 为单射. 等价地, $T$ 为单射, 当且仅当[[向量子空间|核空间]]为零子空间 $\mathrm{Ker} (T) = \{\mathbf{0}\}$. 若 $T$ 在给定基下矩阵表示为 $A \in \mathbb{F}^{m \times n}$, 其中 $n = \dim V$, $m = \dim W$, 则 $A$ 为列满秩矩阵, 即 $\text{rank}(A) = n$. 若 $T$ 为单射, 则 $\dim V \leq \dim W$
