---
创建时间: 2026-02-14
更新时间: 2026-09-05
页面类型: 1
更新次数: 3
准确率: 100
完整度: 85
阅读状态: 0
---
##### 满射线性变换
- 满射线性变换
	- **满射线性变换**是指[[满射]]的[[线性变换]], 在任意基下表示为[[矩阵的秩|行满秩]][[矩阵]]. 设 $T: V \rightarrow W$ 是从向量空间 $V$ 到 $W$ 的线性变换, 若对于目标空间 $W$ 中的任意向量 $\mathbf{w}$, 在定义空间 $V$ 中至少存在一个向量 $\mathbf{v}$ 使得 $T(\mathbf{v}) = \mathbf{w}$, 则称 $T$ 为满射. 等价地, $T$ 为满射, 当且仅当[[像空间]]为目标空间 $\mathrm{Im} (T) = W$. 若 $T$ 在给定基下矩阵表示为 $A \in \mathbb{F}^{m \times n}$, 其中 $n = \dim V$, $m = \dim W$, 则 $A$ 为行满秩矩阵, 即 $\text{rank}(A) = m$. 若 $T$ 为满射, 则 $\dim V\geq \dim W$
		- $\mathrm{Im} (T) = W$


>[!example]- 3维到2维矩阵变换示例
>- $T:\mathbb{R}^3\rightarrow\mathbb{R}^2$, $T(\mathbf{x})=A\mathbf{x}$, $A=\begin{bmatrix} 1 & 0 & 1 \\ 0 & 1 & 1 \end{bmatrix}=\begin{bmatrix} \mathbf{a_1}&\mathbf{a_2}&\mathbf{a_3}\end{bmatrix}$
>	- 定义域 $\mathbb{R}^3$ | 零空间 ${\rm Nul} A=\{c\mathbf{a}\mid c\in\mathbb{R},\mathbf{a}=(-1,-1,1)\}$
>	- 陪域 $\mathbb{R}^2$ | 列空间 ${\rm Col} A=\mathbb{R}^2$
>	- ![[opentext_数学_满射线性变换.png|400]]
>	-  $T(\begin{bmatrix}0\\0\\1\end{bmatrix})=\begin{bmatrix} 1\\1\end{bmatrix}$, $T(\begin{bmatrix}-1\\-1\\1\end{bmatrix})=\begin{bmatrix} 0\\0\end{bmatrix}$
>	- 定义域是 $\mathbb{R}^3$, 定义域中能映射成陪域中 $\mathbf{0}$ 的是一条 $\begin{bmatrix}-1\\-1\\1\end{bmatrix}$ 张成的直线
>	- 陪域是 $\mathbb{R}^2$, 值域也张成 $\mathbb{R}^2$ , 显然满射不是单射

